...
import { EnumButtonType } from 'Objects/UIButton/buttonInterface';
  ...
  screen.titleBar?.buttons.map((button) => {
    ...
    switch (button.type) {
      case EnumButtonType.ORDER:
        pixels = pixelMerge(orderButton(w, h), pixels, x, y);
        break;
      case EnumButtonType.MAXIMIZE:
        pixels = pixelMerge(maximizeButton(w, h), pixels, x, y);
        break;
      default:
        break;
    }
